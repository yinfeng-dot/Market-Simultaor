"""
2026 大型IPO与泡沫风险模拟器 - Streamlit版（含实时数据+趋势预测）
"""
...
with tabs[0]:
    ...
    for label, val in {"AI估值集中": 88, "流动性压力": 79, "盈利能力缺口": 72,
                       "锁定期后抛压": 65, "市场吸收能力": 42}.items():
...
with tabs[4]:
    ...
    if not live_data:
        ...
        fig_live = go.Figure(go.Bar(
            ...
            marker_color=["#A32D2D" if c < 0 else "#0F6E56" for c in changes],
            text=[f"{c:+.2f}%" for c in changes], textposition="outside",
        ))
        fig_live.update_layout(height=320, yaxis_title="涨跌幅 (%)",
...
with tabs[5]:
    ...
    st.caption("基于几何布朗运动 (GBM) 模型，结合市场情绪动态调整漂移率与波动率")
    ...
    st.subheader(f"期末价格分析（T+{time_horizon}月）")
